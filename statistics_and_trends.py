...
def plot_categorical_plot(df):
    ...
    if len(category_cols) == 0 or len(numeric_cols) == 0:
        print("Insufficient categorical columns for the categorical plot.")
        return
    sns.scatterplot(data=df, x=category_cols[0], y=numeric_cols[0], hue=numeric_cols[0])
...
def plot_statistical_plot(df):
    ...
    if len(numeric_cols) == 0:
        print("No numerical columns for the statistical plot.")
        return
    
    sns.histplot(df[numeric_cols[0]], bins=20, kde=True, color='blue')
